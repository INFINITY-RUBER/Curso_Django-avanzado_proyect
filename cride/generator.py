...
for _ in range(n_users):
    user = User.objects.create(email='dummyuser{}@gmail.com'.format(_),
...
for circle in circles:
    for user in users:
        m = Membership.objects.create(user=user,
...
for circle in circles:
    for offerer in random.choices(users, k=random.randint(1, 10)):
        ...
        now = timezone.now()
        departure = now + timedelta(hours=(random.choice([1, -1])) *
                                    random.randint(1, 10))
        ride = Ride.objects.create(offered_by=offerer,
                                   offered_in=circle,
        ...
        passengers = random.choices(users,
                                    k=random.randint(1, available_seats))
