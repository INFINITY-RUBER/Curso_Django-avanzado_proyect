...
class CreateRideRatingSerializer(serializers.ModelSerializer):
    ...
    def create(self, data):
        """Create rating."""
        offered_by = self.context[
            'ride'].offered_by  # es una relacion al usuario
        ...
        ride_avg = round(  # redondea a un dicimal
            Rating.objects.filter(circle=self.context['circle'],
                                  ride=self.context['ride']).aggregate(
                                      Avg('rating'))['rating__avg'], 1)
        self.context['ride'].rating = ride_avg
        self.context['ride'].save()

        user_avg = round(
            Rating.objects.filter(rated_user=offered_by).aggregate(
                Avg('rating'))['rating__avg'], 1)
